...
public class UIVA_Client
{
    ...
    /// <summary>
    /// Get the data from the Razer Hydra Dual Nunchuck controllers
    /// 
    /// The handID parameter specifies which hand controller we are concerned about (left = 0, right = 1)
    /// The format UIVA bounces back is like:
    /// "HYDRA,,posX,posY,posZ,quatX,quatY,quatZ,quatW,trkTS,joyX,joyY,trig,anaTS,MOTRFBJ,buttTS\n"
    /// posX, posY, posZ, quatX, quatY, quatZ, quatW are all double values as 6-DOF tracking of the nunchuck
    /// joyX, joyY are the joystick analog data (-1.0, 1.0), up and right are positive
    /// trig indicates the force sensitive trigger button
    /// M(button Middle), O(One), T(Two), R(Three), F(Four), B(Bumpter), J(Joystick)
    /// Uppercase for pressed, lowercase for released
    /// TS means timestamp for the tracker, the analog, and the buttons
    /// 
    /// </summary>
    /// <param name="which"> Which pair of Hydra is of interest</param>
    /// <param name="handID"> Which hand nunchuck is of interest</param>
    /// <param name="positions"> Hold the returned positions value, must be length 3</param>
    /// <param name="quaternions"> Hold the returned quaternion value, must be length 4</param>
    /// <param name="joystickX"> Joystick value on the X (left and right) axis </param>
    /// <param name="joystickY"> Joystick value on the Y (up and down) axis </param>
    /// <param name="trigger"> The value of the trigger analog button </param>
    /// <param name="buttons"> The buttons information </param>
    public void GetHydraNunchuckData(int which, int handID, ref double[] positions, ref double[] quaternions,
        out double joystickX, out double joystickY, out double trigger, out String buttons)
    {
        if (positions.Length != 3 || quaternions.Length != 4) {
            throw new Exception("ERROR: \"positions\" length must be 3 and \"quaternions\" length must be 4");
        }
        SendMessage(String.Format("Hydra?{0}?{1}?\n", which, handID));
        ReceiveMessage();
        try {
            string[] sections = recStr.Split(new char[] { ',' });
            //Skip "PHASESPACE,,"
            ...
            quaternions[2] = System.Convert.ToDouble(sections[7]);
            quaternions[3] = System.Convert.ToDouble(sections[8]);
            //Skip the tracker timestamp, uncomment this line and add a corresponding out DateTime variable to test time delay
            //trackTS = DateTime.Parse(sections[9]);
            joystickX = System.Convert.ToDouble(sections[10]);
            joystickY = System.Convert.ToDouble(sections[11]);
            trigger = System.Convert.ToDouble(sections[12]);
            //Skip the analog timestamp, uncomment this line and add a corresponding out DateTime variable to test time delay
            //anaTS = DateTime.Parse(sections[13]);
            buttons = sections[14];
            //Skip the buttons timestamp, uncomment this line and add a corresponding out DateTime variable to test time delay
            //buttTS = DateTime.Parse(sections[15]);
        }
        catch (Exception e) {
            throw new Exception(e.ToString() + "\n\nRECEIVED FROM UIVA_SERVER: " + recStr);
        }
    ...
    /* An overload which requires no 'which' parameter, used when there is only one phasespace */
    public void GetHydraNunchuckData(int handID, ref double[] positions, ref double[] quaternions,
        out double joystickX, out double joystickY, out double trigger, out String buttons)
    {
        GetHydraNunchuckData(1, handID, ref positions, ref quaternions, out joystickX, out joystickY, out trigger, out buttons);
    }
