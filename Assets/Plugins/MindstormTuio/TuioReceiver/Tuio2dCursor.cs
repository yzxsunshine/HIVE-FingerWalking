...
namespace Tuio
{
	public class Tuio2DCursor
	{
	    int sessionID;
		int fingerID, padID;
	    float posX, posY, velX, velY, acc, force;
	    ...
	    public Tuio2DCursor(OSCMessage message)
	    {
			...
			this.FingerID = Convert.ToInt32(message.Values[2]);
			this.PadID = Convert.ToInt32(message.Values[3]);
	        this.PositionX = Convert.ToSingle(message.Values[4]);
	        this.PositionY = Convert.ToSingle(message.Values[5]);
	        this.VelocityX = Convert.ToSingle(message.Values[6]);
	        this.VelocityY = Convert.ToSingle(message.Values[7]);
	        this.Acceleration = Convert.ToSingle(message.Values[8]);
			this.Force = Convert.ToSingle(message.Values[9]);
	    }
	    #endregion
	    ...
	    public OSCMessage GetMessage()
	    {
			...
			msg.Append(this.FingerID);
			msg.Append(this.PadID);
	    ...
	    public bool IsEqual(Tuio2DCursor cursor)
	    {
	        return this.Acceleration == cursor.Acceleration
				...
				&& this.FingerID == cursor.FingerID
				&& this.PadID == cursor.PadID
	            && this.VelocityX == cursor.VelocityX
	            && this.VelocityY == cursor.VelocityY
				&& this.Force == cursor.Force;
	    }
	}
}